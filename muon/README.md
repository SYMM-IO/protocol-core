...
uPnlPartyA: async function (partyA, chainId, v3Contract) {
    ...
    const { openPositions, quoteIds } = await this.fetchOpenPositions({ partyA }, 'A', chainId, v3Contract);
    ...
    // nonce, quote IDs, open positions, and prices map (retrieved using the getPrices function)
    if (openPositions.length == 0) {
        return {
            ...
            quoteIds,
            openPositions,
            pricesMap: await this.getPrices()
        };
    }
    ...
    // Returns the result with the calculated uPnl, notional value sum, nonce, prices map, prices, quote IDs, and open positions
    return {
        uPnl: uPnl.toString(),
...
fetchPrices: async function (quoteIds, chainId, v3Contract) {
  ...
  const pricesMap = await this.getPrices();
...
getPrices: async function () {
    ...
    data.forEach((el) => {
        // Extract the symbol and scaled mark price
        const symbol = el.symbol;
        const scaledMarkPrice = scaleUp(el.markPrice);

        // Convert the mark price to a string and store it in the prices map
        pricesMap[symbol] = scaledMarkPrice.toString();
    });
