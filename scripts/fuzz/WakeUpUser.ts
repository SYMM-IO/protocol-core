...
async function main() {
	...
	const balance = Number(process.env.TEST_USER_BALANCE != null ? process.env.TEST_USER_BALANCE : "5000")
	await user.setBalances(decimal(balance), decimal(balance), decimal(balance))
	const userController = new UserController(context.manager, user, undefined)
	await userController.start()
	const maxLockedAmountForQuote = Number(process.env.TEST_USER_MAX_QUOTE != null ? process.env.TEST_USER_MAX_QUOTE : "100")
