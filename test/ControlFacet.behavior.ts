...
export function shouldBehaveLikeControlFacet(): void {
	...
	describe("setPartyBWhitelistedSymbolTypeStatus", () => {
		it("Should setPartyBWhitelistedSymbolTypeStatus successfully", async function () {
			expect(await context.viewFacet.getPartyBWhitelistedSymbolTypeStatus(context.signers.hedger, 2)).to.be.equal(false)
			await expect(context.controlFacet.connect(owner).setPartyBWhitelistedSymbolTypeStatus(context.signers.hedger, 2, true)).to.not.reverted
			expect(await context.viewFacet.getPartyBWhitelistedSymbolTypeStatus(context.signers.hedger, 2)).to.be.equal(true)
			await expect(context.controlFacet.connect(owner).setPartyBWhitelistedSymbolTypeStatus(context.signers.hedger, 2, false)).to.not.reverted
			expect(await context.viewFacet.getPartyBWhitelistedSymbolTypeStatus(context.signers.hedger, 2)).to.be.equal(false)
		})
	})
}
