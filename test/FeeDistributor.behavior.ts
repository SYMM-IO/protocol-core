	...
	describe("setStakeholders", function () {
		it("Should allow manager to set stakeholders", async function () {
			let newStakeholders = [
				...
				{receiver: stakeholder2.address, share: ethers.utils.parseEther("0.1")},
				{receiver: stakeholder2.address, share: ethers.utils.parseEther("0.1")}
			]
			...
			expect((await feeCollector.stakeholders(2)).share).to.equal(ethers.utils.parseEther("0.1"))
			expect((await feeCollector.stakeholders(3)).receiver).to.equal(stakeholder2.address)
			expect((await feeCollector.stakeholders(3)).share).to.equal(ethers.utils.parseEther("0.1"))

			newStakeholders = [
				{receiver: stakeholder1.address, share: ethers.utils.parseEther("0.1")},
				{receiver: stakeholder2.address, share: ethers.utils.parseEther("0.4")}
			]
			await feeCollector.connect(manager).setStakeholders(newStakeholders)
			expect((await feeCollector.stakeholders(1)).receiver).to.equal(stakeholder1.address)
			expect((await feeCollector.stakeholders(1)).share).to.equal(ethers.utils.parseEther("0.1"))
			expect((await feeCollector.stakeholders(2)).receiver).to.equal(stakeholder2.address)
			expect((await feeCollector.stakeholders(2)).share).to.equal(ethers.utils.parseEther("0.4"))
			await expect(feeCollector.stakeholders(3)).to.be.reverted
		})
